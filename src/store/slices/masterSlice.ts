...
const masterSlice = createSlice({
  ...
  initialState,
  reducers: {
    addPaymentMode: (state, action: PayloadAction<Omit<PaymentMode, 'id'>>) => {
      const newMode = {
        ...action.payload,
    ...
    updatePaymentMode: (state, action: PayloadAction<PaymentMode>) => {
      const index = state.paymentModes.findIndex(mode => mode.id === action.payload.id);
      if (index !== -1) {
        state.paymentModes[index] = action.payload;
      }
    },
    deletePaymentMode: (state, action: PayloadAction<string>) => {
      state.paymentModes = state.paymentModes.filter(mode => mode.id !== action.payload);
    },
    ...
    addDesignation: (state, action: PayloadAction<Omit<Designation, 'id'>>) => {
    ...
    updateDesignation: (state, action: PayloadAction<Designation>) => {
      const index = state.designations.findIndex(desig => desig.id === action.payload.id);
      if (index !== -1) {
        state.designations[index] = action.payload;
      }
    },
    deleteDesignation: (state, action: PayloadAction<string>) => {
      state.designations = state.designations.filter(desig => desig.id !== action.payload);
    },
  },
});
